...
@Test func bundledSlackManifestEnablesPKCEForCustomCallback() {
    ...
    #expect(SlackAppManifest.template.contains("users:read"))
    #expect(SlackAppManifest.template.contains("users:read.email"))
    #expect(SlackAppManifest.template.contains("reactions:read"))
    #expect(SlackAppManifest.template.contains("emoji:read"))
...
@Test func callbackRequiresMatchingStateAndRoute() throws {
    ...
    #expect(authorizationItems.first(where: { $0.name == "user_scope" })?.value == SlackOAuth.requestedUserScopes)
...
@Test func managedProfileContainsOnlyPublicLegalConfigurationAndAppliesOnLaunch() throws {
    ...
    #expect(values[ManagedConfiguration.Key.version] as? Int == ManagedConfiguration.configurationVersion)
    #expect(Set(try #require(values[ManagedConfiguration.Key.requiredUserScopes] as? [String])) == SlackOAuth.requiredUserScopes)
    #expect(values["ThreadLightRequiredScope"] == nil)
