...
Development builds keep OAuth tokens in ThisDeviceOnly Keychain items, but replace database-key and Secure Enclave persistence with file-protected local keys and ephemeral signatures. They use separate storage namespaces and are not valid production evidence tools.
