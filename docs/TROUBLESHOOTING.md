...
| `no_bot_scopes_requested` during installation | Installation started from the wrong Slack page. | Open the specific app, then use **Settings → Install App → Install to Organization**. |
...
| OAuth callback does not match | Slack and ThreadLight have different redirect values. | Use exactly `threadlight://oauth/callback`. |
